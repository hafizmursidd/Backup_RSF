...
namespace GST00500Front
{
    public partial class GST00500RejectPopUp
    {
        ...
        private async Task OnClickOKRejectButton()
        {
            ...
            try
            {
                if ((string.IsNullOrEmpty(_viewModelGST00500Inbox.ParamTransactionStatus.CREASON_CODE)) ||
                (string.IsNullOrEmpty(_viewModelGST00500Inbox.ParamTransactionStatus.TNOTES)))
                {
                    loEx.Add(new Exception(""));
                }
                else
                {
                    await _viewModelGST00500Inbox.SaveApprovalBatch(false);
                    if (!string.IsNullOrEmpty(_viewModelGST00500Inbox.ResultFailedTransaction))
                    {
                        R_MessageBox.Show("",
                            $"Reject Transaction With Company {_viewModelGST00500Inbox.ResultFailedTransaction} has been failed when update status Canceled",
