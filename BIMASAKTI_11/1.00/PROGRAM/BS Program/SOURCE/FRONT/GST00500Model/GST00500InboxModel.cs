...
namespace GST00500Model
{
    public class GST00500InboxModel : R_BusinessObjectServiceClientBase<GST00500DTO>, IGST00500
    {
        ...
        public async Task<GST00500ApprovalTransactionListDTO> SaveTransactionListAsync()
        {
            var loEx = new R_Exception();
            ...
            {
                R_HTTPClientWrapper.httpClientName = _HttpClientName;
                loReturn = await R_HTTPClientWrapper.R_APIRequestObject<GST00500ApprovalTransactionListDTO>(
                    _RequestServiceEndPoint,
                    nameof(IGST00500.SaveTransactionWithReturn),
                    DEFAULT_MODULE,
                    _SendWithContext,
                    _SendWithToken);
            }
